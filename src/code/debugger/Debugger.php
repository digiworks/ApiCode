...
class Debugger implements ServiceInterface, DebuggerInterface {
    ...
    /**
     * Retrieves the code coverage analysis data stored in the Debugger::$_finalCoverageData property
     */
     ...
     * @param 	array 		$options		array of options, see Debugger::$ _defaultOptions
     */
    public static function load($options = null) {
        $now = microtime(true);
        if (defined('_DEBUGGER_NAMESPACE_')) { // check if the debug class is already loaded
        ...
        if (!@$_SESSION['debugger']) {
            $_SESSION['debugger'] = array();
        }
        if (@$_GET[static::$_options['url_key']] == static::$_options['url_pass']) {
            $_SESSION['debugger'][static::$_options['url_key']] = true;
            $_SESSION['debugger']['code_highlighter'] = true;
            $_SESSION['debugger']['search_files'] = true;
            //$buffer .= '<br>Debugger turned on!';
        } else if (@$_GET[static::$_options['url_key'] . '_off'] == static::$_options['url_pass']) {
            $_SESSION['debugger'][static::$_options['url_key']] = false;
            $_SESSION['debugger']['code_highlighter'] = false;
            $_SESSION['debugger']['search_files'] = false;
        }
        if (static::_getSessionVars(static::$_options['url_key'])) {
            ...
            @define('_DEBUGGER_NAMESPACE_', $called_class);
    ...
    /**
     * Exclude Debugger::$_buildBuffer from execution timing property
     */
     ...
     */
    protected static $_panelCss = '#DebuggerPanel{font-family:Arial,sant-serif;
				...
				#DebuggerPanel ul.tabs li{background-color:#ddd;border-color:#999;margin:0 -3px -1px 0;
				padding:3px 6px;border-width:1px;list-style:none;display:inline-block;border-style:solid;}
				#DebuggerPanel ul.tabs li.active{background-color:#fff;border-bottom-color:transparent;
				text-decoration:}#DebuggerPanel ul.tabs li:hover{background-color:#eee;}
				#DebuggerPanel ul.tabs li.active:hover{background-color:#fff;}
				#DebuggerPanel ul.tabs.merge-up{margin-top:-24px;}
				#DebuggerPanel ul.tabs.right{padding:0 0 0 0;text-align:right;}
				#DebuggerPanel ul.tabs{border-bottom-color:#999;border-bottom-width:1px;font-size:14px;
				list-style:none;margin:0;padding:0;z-index:100000;position:relative;
				background-color:#EEE}#DebuggerPanel ul.tabs a{color:purple;font-size:10px;
				text-decoration:none;}#DebuggerPanel .tabs a:hover{color:red;}
				#DebuggerPanel ul.tabs a.active{color:black;background-color:yellow;}
				...
				#DebuggerFilterBar{background-color:black;margin-bottom:8px;padding:4px;
				...
				#DebuggerPanelTitle{height:25px;float:left;z-index:1000000;position:relative;}
				#DebuggerPanelTitle h1{font-size:16px;font-weight:bold;margin-bottom:20px;
				...
				#DebuggerStatusBar{height:2px;background-color:#999;}';
    ...
    protected static function _setSessionVars() {
        $_SESSION['debugger']['show_messages'] = static::$_options['show_messages'];
        $_SESSION['debugger']['show_globals'] = static::$_options['show_globals'];
        $_SESSION['debugger']['show_sql'] = static::$_options['show_sql'];
        $_SESSION['debugger']['show_w3c'] = static::$_options['show_w3c'];
    }
    ...
    protected static function _disablePanels() {
        ...
        if (!@empty($hide)) {
            $_SESSION['debugger']['show_messages'] = true;
            $_SESSION['debugger']['show_globals'] = true;
            $_SESSION['debugger']['show_sql'] = true;
            $_SESSION['debugger']['show_w3c'] = true;
            foreach ($hide as $k => $v) {
                if ($v == 'msg' || $v == 'all') {
                    $_SESSION['debugger']['show_messages'] = false;
                }
                if ($v == 'globals' || $v == 'all') {
                    $_SESSION['debugger']['show_globals'] = false;
                }
                if ($v == 'sql' || $v == 'all') {
                    $_SESSION['debugger']['show_sql'] = false;
                }
                if ($v == 'w3c' || $v == 'all') {
                    $_SESSION['debugger']['show_w3c'] = false;
                }
            }
        }
        static::$_options['show_messages'] = static::_getSessionVars('show_messages');
    ...
    protected static function _buildBuffer($type, $string, $statement = null, $category = null) {
        ...
        if (defined('_DEBUGGER_NAMESPACE_') &&
     ...
     * @return	the trace without the methods in the Debugger::$_excludeMethods property
     */
    protected static function _debugTrace($depth = NULL) {
        ...
        foreach ($raw_trace as $k => $arr) {
            if ((@$arr['class'] == 'Debugger' && (@preg_match("|_|", @$arr['function']) ||
    ...
    protected static function _buildInterface() {
        ...
        $interface .= '<div id="DebuggerPanel">';
        $interface .= '<div id="DebuggerPanelTitle" style="display:none;">&nbsp;</div>';
    ...
    protected static function _buildHtmlTable($type) {
        ...
        if (@static::$_buffer[$type]) {
            ...
            if (sizeof($categories) > 1) {
                ksort($categories);
                $div .= '<div id="DebuggerFilterBar"><a href="#" onClick="ptc_filter_categories(\'' .
        ...
        return static::_compressHtml(
					...
					function ptc_show_panel(elId,panelTitle,el)
					{
						...
						{ 	
							...
							document.getElementById(\'DebuggerStatusBar\').style.display=\'\';
							document.getElementById(floatDivId).style.width=\'100%\';
							el.className="active";activePanelID=elId;ptc_set_title(panelTitle);
						}
						else
						{
							document.getElementById(\'DebuggerPanelTitle\').style.display=\'none\';
							ptc_reset_panels();
					...
					{
						document.getElementById(\'DebuggerStatusBar\').style.display=\'none\'; 
						for(var i in panels){document.getElementById(panels[i]).style.display=\'none\';}
					};
					function ptc_set_title(panelTitle)
					{
						document.getElementById(\'DebuggerPanelTitle\').style.display=\'\';
						document.getElementById(\'DebuggerPanelTitle\').innerHTML=\'<h1>\'+panelTitle+\'</h1>\';
					};
					function hideInterface(){document.getElementById(\'DebuggerPanel\').style.display=\'none\';};
					...
					function ptc_read_code(filename,line) 
					{
                        ...
                        $path = addslashes(str_replace(realpath($_SERVER['DOCUMENT_ROOT']),
                                        '', realpath(dirname(__FILE__)))) . '/Debugger.php?ptc_read_file="+filename;
					...
					function ptc_search_string( )
					{
                        ...
                        '/Debugger.php?ptc_search_files="+document.getElementsByName("ptc_search_files")[0].value;
					...
					window.onload=function( ) 
					{
						var div=document.getElementById("DebuggerStatusBar");
						var press=false;
					...
					{
						var floatDivId="DebuggerPanel";resetPanels();
    ...
    /**
     * Retrieves the session var for the debugger class
    ...
    public static function showSearchPopup($string, $path = null) {
        ...
        $result .= Debugger::findString($string, $path);
...
if (@$_GET['ptc_read_file']) {
    @session_start();
    if (!@$_SESSION['debugger']['code_highlighter']) {
        exit();
    }
    echo Debugger::highlightFile($_GET['ptc_read_file'], @$_GET['ptc_read_line']);
    exit();
}
/**
...
if (@$_GET['ptc_search_files']) {
    @session_start();
    if (!@$_SESSION['debugger']['search_files']) {
        exit();
    }
    echo Debugger::showSearchPopup($_GET['ptc_search_files'], @$_GET['ptc_search_path']);
    exit();
}